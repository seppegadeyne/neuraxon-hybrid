...
Verdict: `source semantics audit: output-port teacher forcing unsupported; input-proxy target drive observable but motor readout baseline-level; external-drive window probe keeps hidden/output drive non-teacher-forcing; VRAM-resident 4h run flat`

## What changed

The previous cuNxon slices established CUDA runtime viability, long-horizon raw dynamics, a tiny task-coupled action probe, infer-vs-train sensitivity, hidden-state/pattern inspection, a three-sphere action adapter, a longer task-coupled sweep over 108 live RTX 5090 samples, an interface-semantics probe for absolute output-neuron ports, a supervised motor-target adapter, and a source-level C/CUDA audit. This follow-up adds `benchmarks/results/cunxon_external_drive_window_probe.*`: a controlled ctypes probe that drives the same external values through input, hidden, and output sensory-id windows in both `StepInfer` and `StepTrain`. It also preserves `benchmarks/results/cunxon_input_proxy_target_probe.*`, where target drive was moved from unsupported output-neuron ports to supported input-class external-drive ports, and the bounded `cunxon-vram-resident` four-hour artifact under `benchmarks/results/cunxon_vram_resident_run.*`.
